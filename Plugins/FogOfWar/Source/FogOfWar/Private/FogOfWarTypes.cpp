...
//     const TFunctionRef<FogOfWarTypes::GridIndexType(const FVector2D&)> GetGridIndexFunction, // 改为 TFunction
//     const FQuat& BoxQuat)
...
//     // 2. 如果初始位置刚好落在 INDEX_NONE 区域，自动推进到第一个有效网格！
...
//     // do-while 保证【至少向前推进一步】，然后跳过后续所有的 INDEX_NONE
//     do
//     {
//         CurrentPosition.X += GridSize.X;
        
//         // 当 X 步进超出右边界时，换行重置 X 并增加 Y
...
// } while (IsInsideOrOnXY() && operator*() == INDEX_NONE); // 遇到 INDEX_NONE 自动在内部循环跳过
...
//     // 关键修正：只判断是否还在包围盒范围内！绝对不能在这里判断 operator*() == INDEX_NONE
...
//     // 加上 1e-4 的 Epsilon 容差，防止浮点数累加微小误差导致边界测试提前失效
